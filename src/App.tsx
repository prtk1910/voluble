...
import { Archive, CalendarClock, CheckSquare2, ChevronDown, CircleAlert, FileText, GitMerge, ListChecks, Menu, Mic, NotebookPen, Plus, Search, Settings as SettingsIcon, ShoppingBasket, Sparkles, Tag, X } from 'lucide-react';
import { ApiError, api } from './api/client';
import { categories, createRecord, type Category, type VolubleRecord } from './domain/record';
...
import { cacheRecords, cachedRecords, pendingOperations, readState, removeOperation, writeState } from './sync/outbox';
...
const initialPreferences: Preferences = { transcription: 'local', cleanup: 'openai', language: 'en-US' };
...
export default function App() {
  ...
  const [preferences, setPreferences] = useState<Preferences>(() => {
    try { return JSON.parse(localStorage.getItem('voluble-preferences') ?? '') as Preferences; } catch { return initialPreferences; }
  });

  const load = async () => {
    let authenticated = false;
    try {
      const local = await cachedRecords(); if (local.length) setRecords(local); cacheReady.current = true;
      ...
      if (current.drive.state === 'disconnected') { setSync('disconnected'); setMessage('Google authorization expired. Reconnect to resume remote writes; your local text is safe and read-only.'); }
        ...
        const merged = remote.incremental ? [...local.filter((item) => !remote.removed.includes(item.drive?.fileId ?? '') && !remote.records.some((changed) => changed.id === item.id)), ...remote.records] : remote.records;
  ...
  const chooseFolder = async () => { try { const id = await chooseDriveFolder(); if (id) { await api.selectFolder(id); await load(); setMessage('Drive folder connected.'); } } catch (error) { setMessage(error instanceof Error ? error.message : 'Folder selection failed.'); } };
  ...
  return <div className="app-shell">{nav}<main className="content"><header className="mobile-header"><button className="icon-button" onClick={() => setMobileNav(true)}><Menu /></button><Brand /><button className="icon-button" onClick={() => setRecording(true)}><Mic /></button></header>{message && <div className={`banner ${sync === 'disconnected' ? 'error' : ''}`}><CircleAlert /> {message}{sync === 'disconnected' && <a className="button-link" href="/api/auth/login">Reconnect</a>}<button onClick={() => setMessage('')}><X /></button></div>}
    {view === 'Settings' ? <Settings preferences={preferences} onPreferences={setPreferences} folderId={session.drive.folderId} onChooseFolder={() => void chooseFolder()} onSignedOut={() => setSession(undefined)} /> : view === 'Conflicts' ? <ConflictCenter conflicts={conflicts} onResolve={resolve} /> : <>
      ...
      {!session.drive.folderId && <div className="onboarding"><CloudIcon /><div><strong>Choose your Voluble folder</strong><p>Records will be saved as Markdown and ICS files in a folder you select. Voluble will never silently switch it.</p></div><button className="primary" onClick={() => void chooseFolder()}>Choose Drive folder</button></div>}
      ...
      {shown.length ? <div className="record-grid">{shown.map((record) => <article className="record-card" key={record.id} onClick={() => sync === 'disconnected' ? setMessage('Cached Drive records are read-only until you reconnect Google. New unsynchronized captures remain safe locally.') : setEditing(record)}><div className="record-meta"><span className={`category-dot c-${categories.indexOf(record.category)}`} />{record.category}<time>{new Date(record.updatedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</time></div><h2>{record.title}</h2><p>{record.content || record.originalTranscript || 'No text yet'}</p>{record.tags.length > 0 && <div className="tags"><Tag />{record.tags.slice(0, 3).map((tag) => <span key={tag}>{tag}</span>)}</div>}<footer><span className={`status ${record.status}`}>{record.status.replace('-', ' ')}</span>{record.event && <div className="calendar-actions"><button onClick={(event) => { event.stopPropagation(); window.open(googleCalendarUrl(record), '_blank', 'noopener'); }}>Google Calendar</button><button onClick={(event) => { event.stopPropagation(); download(`${record.title}.ics`, generateIcs(record), 'text/calendar'); }}>ICS</button></div>}</footer></article>)}</div> : <div className="empty-state"><NotebookPen /><h2>{query ? 'No matching records' : 'Nothing here yet'}</h2><p>{query ? 'Try a different phrase or category.' : 'Capture a thought and Voluble will turn it into a useful record.'}</p><button className="primary" onClick={() => setRecording(true)}><Mic /> Start a capture</button></div>}
    </>}</main>{editing && <RecordEditor record={editing} onSave={(value) => void save(value)} onTrash={(value) => void trash(value)} onClose={() => setEditing(undefined)} />}{recording && <RecorderPanel provider={preferences.transcription} cleanupProvider={preferences.cleanup} language={preferences.language} onRecord={(value) => void save(value)} onClose={() => setRecording(false)} />}</div>;
}
