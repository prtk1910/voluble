...
export default function App() {
  ...
  const retryProcessing = async (record: VolubleRecord) => {
    ...
    try {
      const { result, model, provider, freeTasksRemaining } = await api.cleanup(preferences.cleanup, transcript, record.language, preferences.timezone, record.id);
      ...
      else if (error instanceof ApiError && error.code === 'free_credit_deducted') void api.providerStatus().then(setProviderStatus).catch(() => undefined);
  ...
  return <div className="app-shell">{nav}<main className="content"><header className="mobile-header"><button className="icon-button" onClick={() => setMobileNav(true)}><Menu /></button><Brand /><button className="icon-button" onClick={startCapture}><Mic /></button></header>{message && <div className={`banner ${sync === 'disconnected' ? 'error' : ''}`}><CircleAlert /> {message}{sync === 'disconnected' && <a className="button-link" href="/api/auth/login">Reconnect</a>}<button onClick={() => setMessage('')}><X /></button></div>}
    {session.drive.folderId && !providerStatus.providers.length && <div className="onboarding provider-onboarding"><KeyRound /><div><strong>{providerStatus.freeTierAvailable && providerStatus.freeTasksRemaining > 0 ? `${providerStatus.freeTasksRemaining} free note${providerStatus.freeTasksRemaining === 1 ? '' : 's'} remaining` : 'Add your provider API key'}</strong><p>{providerStatus.freeTierAvailable && providerStatus.freeTasksRemaining > 0 ? 'Voluble will use its provider for your first 10 notes. After that, add your own OpenAI or Gemini key to continue.' : providerStatus.freeTierAvailable ? 'Your 10 free notes are finished. Add an OpenAI or Gemini API key to keep creating notes.' : 'Add an OpenAI or Gemini API key in Settings before starting your first capture.'}</p></div><button className="primary" onClick={() => { setView('Settings'); setMobileNav(false); }}>Open Settings</button></div>}
