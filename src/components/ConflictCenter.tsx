...
export function ConflictCenter({ conflicts, onResolve }: { conflicts: Conflict[]; onResolve(conflict: Conflict, records: VolubleRecord[]): void }) {
  ...
  if (!conflicts.length) return <div className="empty-state"><GitMerge /><h2>No unresolved conflicts</h2><p>If two devices edit the same Drive record, both versions will be preserved here.</p></div>;
